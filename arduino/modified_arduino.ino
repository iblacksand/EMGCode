This is a smart architectural shift. Offloading the peak detection to the server significantly simplifies your Arduino code, reduces the computational overhead on the microcontroller, and centralizes your analytical logic where it's easier to tweak and update.

Here is the fully modified and calibrated code based on your new two-phase architecture.

### Key Changes Implemented:

* **Removed Local Detection:** `current_batch_max`, `peak_detected`, and local classification thresholds have been stripped out of the main loop.
* **Calibration Phase:** Added a state machine that handles the 3-step calibration. It flashes blue while collecting data, sends the batch to `/api/arduino/calibration_signal`, and provides the requested 1-second green (success) or blinking red (fail) feedback before trying again.
* **Live Reading Phase:** Evaluates the `analogRead` in real-time against the fetched thresholds to set the LED.
* **Network Indicator:** Added a direct blue LED override right before `HTTP POST` requests execute, which immediately hands control back to the real-time sensor color map once the data is sent.
